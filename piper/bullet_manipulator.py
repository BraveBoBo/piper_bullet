...
class ManipulatorInfo:
    def __init__(self, robot_id, joint_ids, joint_names,
        ...
        self.finger_jids_lst = finger_jids_lst #different from arm_jids_lst TODO: check
...
class BulletManipulator:
    ...
    def __init__(self, sim, robot_desc_file, ee_joint_name, ee_link_name,
        ...
        self.kp = kp; self.kd = kd; self.min_z = min_z# for safety
        self.debug = debug
        self.sim = sim
    ...
    def reset(self):# Always reset to rest position.
        self.reset_to_qpos(self.rest_qpos)

    def reset_to_qpos(self, qpos):# the function that resets the robot to a specific position.
    ...
    def clear_motor_control(self):# Clear motor control.
    ...
    def set_joint_limits(self, minpos, maxpos): # Set joint limits.
    ...
    def get_max_fing_dist(self): # what is the maximum finger distance?
        farr = np.array(self.info.finger_jids_lst)
        return self.info.joint_maxpos[farr].sum() # for symmetric commands

    def get_qpos(self):# get the current joint positions.
    ...
    def get_qvel(self): #  get the current joint velocities.
        joint_states = self.sim.getJointStates(
            self.info.robot_id, self.info.joint_ids)
    ...
    def get_ee_pos(self, left=False):# get the end effector position.
