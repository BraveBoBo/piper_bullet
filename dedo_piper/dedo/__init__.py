...
for task, versions in TASK_INFO.items():
    ...
    register(id=task+'-v'+str(0), entry_point='dedo.envs:'+cls_nm, order_enforce=False,disable_env_checker=True)
    for version_id, obj_name in enumerate(versions):
        register(id=task+'-v'+str(version_id+1),
                 entry_point='dedo.envs:'+cls_nm, order_enforce=False,disable_env_checker=True)
