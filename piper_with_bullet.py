...
class BulletManipulator:
    ...
    def __init__(self, sim, robot_desc_file, ee_joint_name, ee_link_name,
        ...
        if not os.path.isabs(robot_desc_file):
            ...
            robot_desc_file = os.path.join(REPOSITORY_PATH, 'src', 'piper_description', 'urdf', robot_desc_file)
...
if __name__ == "__main__":
    ...
    # Load the mobile manipulator.
    base_pos = [0, 0, 0.5]
    base_quat = [0, 0, 0, 1]
    robot = BulletManipulator(sim, 'mobile_manipulator.urdf',
                              'ee_joint', 'ee_link', 'ee_position',
                              base_pos, base_quat, 1.0)
